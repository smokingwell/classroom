...
&usage("$0: too many arguments") if @ARGV > 2;
...
sub usage {
  warn "@_\n" if @_;
  die "usage: $0: [-v] [-i] [-f LOGFILE] REGEXP [WCPATH|URL]\n",
      "\n",
      "Print only log messages matching REGEXP, either by running 'svn log'\n",
      "in the current working directory or to specify WCPATH or URL, or if '-f LOGFILE' is passed, then\n",
