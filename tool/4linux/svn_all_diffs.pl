...
# $HeadURL: http://svn.collab.net/repos/svn/trunk/contrib/client-side/svn_all_diffs.pl $
# $LastChangedDate: 2008-03-25 22:42:32 +0800 (Tue, 25 Mar 2008) $
...
while (@revisions > 1)
  {
    ...
    if ($quietmode == 0){
      print "Press y to continue,n to stop,y/n(y):";
      chomp($input=<STDIN>);
    }
    last if ($input eq "n");
  }
...
#  print "\n\n\nRevision $revision\n";
  system("svn log -v --incremental -r$revision .");
}
